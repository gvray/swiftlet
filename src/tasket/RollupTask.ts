...
class RollupTask extends SwiftletTask {
  ...
  constructor(options: RollupOptions, hooks?: CompilerHooks) {
    ...
    const { output, watch, ...inputLike } = options;
    const out = output as OutputOptions | OutputOptions[];
    this.outputOptionsList = Array.isArray(out) ? out : out ? [out] : [];
    this.watchOptions = watch as WatcherOptions;
    ...
    try {
      ...
      if (hasWatch) {
        ...
        const watcher: RollupWatcher = watch({
          ...
          watch: options.watch || this.watchOptions || false,
        });
        const format = this.outputOptionsList?.[0]?.format
          ? String(this.outputOptionsList?.[0]?.format)
          : 'unknown';
        this.hooks?.status.call({
          message: 'Entering watch mode...',
          scope: 'watch',
          phase: 'build',
        });
        watcher.on('event', (event: RollupWatcherEvent) => {
          switch (event.code) {
            ...
            case 'BUNDLE_END': {
              ...
              this.hooks?.status.call({
                message: `Compiled in ${duration}ms`,
                scope: 'watch',
                phase: 'finalize',
              });
              break;
