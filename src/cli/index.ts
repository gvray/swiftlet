...
export async function run({
  ...
  pck: { name: string; description?: string; version: string };
}) {
  ...
  program
    ...
    .option('--splitting', 'Enable code splitting / multi-file output')
    .option('--preserve-modules', 'Preserve source module directory structure in output')
    .option(
      '--entry-file-names <pattern>',
      'Custom entry chunk file name pattern (multi-file mode)'
    )
    .option(
      '--external <pkg>',
    ...
    .action(async (args) => {
      try {
        ...
        if (!mainConfigFile)
          throw Error(
            'Check if your project is missing a configuration file (swiftlet.config.(ts|js))'
          );
        ...
        if (args.printRollup) {
          ...
          const printable = formatRollupForPrint(
            Array.isArray(rollupConfigs) ? rollupConfigs : [rollupConfigs]
          );
